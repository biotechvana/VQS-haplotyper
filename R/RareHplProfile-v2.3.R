...
###  Carrega rare.w, Tnr, Tnh, log.cuts
load(file.path(repDir,"RareHplWeight.RData"))
###  Carrega descriptors de mostres
samples <- fread(file.path(dataDir,"samples.csv"), sep="auto", header=T,
                 stringsAsFactors=F)
### Sincronitzar samples amb les altres estructures
...
# for(paseT in ptrt)
# {
# idx <- which(samples$pBase==paseB & samples$Treat==treat & 
# samples$pTreat==paseT)
# if(length(idx)<2) next
# M <- rare.w[idx,]*100
# plot(log.cuts,M[1,],type="l",col=pal[1],xlab="log cut",ylab="Weight (%)",
# ylim=c(0,100))    
# sapply(2:nrow(M),function(k) lines(log.cuts,M[k,],col=pal[k]))
# legend("topleft",lwd=3,col=pal,legend=samples$Primer.ID[idx],cex=0.8)
# title(main=paste(paseB,treat,paseT,sep=" - "))
# }
# }  
# }
...
for (i in 1:length(unique(samples$Primer.ID))) {
  ...
  #seM <- seM[o,]
  nms <- paste(samples$pBase[idx],samples$Treat[idx],samples$pTreat[idx],sep=".")
  #nms <- nms[o]
