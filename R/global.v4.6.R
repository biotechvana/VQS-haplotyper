...
###    - Pairwise differences including gaps              ###
###                                                       ###
#############################################################
library(Biostrings)
...
###    given sequence frequencies
SummaryMuts.w <- function(seqs,w,off=0, RefSeq)
{ if(length(seqs)<2)
    return(NULL)
  psa <- pairwiseAlignment(seqs, seqs[[1]])
  aln <- as.character(aligned(psa))
  pos.tbl <- SeqsTbl.w(aln,w)
  mut.tbl <- MutsTbl(pos.tbl)
  flags <- apply(mut.tbl,1,sum)>0
  pos <- which(flags)
  if(length(pos)==1)
  { res <- c(pos=pos+off,substr(RefSeq, pos, pos),pos.tbl[flags,],
               round(pos.tbl[flags,]/sum(pos.tbl[1,])*100,3))
    res <- matrix(res,nrow=1)
  } else {	  
    ances_allele <- c()
    for (p in pos) {
      nt <- substr(RefSeq, p, p)
      ances_allele <- c(ances_allele, nt)
    }
    res <- cbind(pos=pos+off,ances_allele,pos.tbl[flags,],
                 round(pos.tbl[flags,]/sum(pos.tbl[1,])*100,3))
  }
  rownames(res) <- 1:nrow(res)
