...
# cf1606D.AXIN1.191.341.fna
fl.patt <- "\\.fna$"

flnms <- list.files(path=nt01Dir,patt=fl.patt)
...
for(i in 1:n)
{
   lst <- get.data(file.path(nt01Dir,flnms[i]),0)   
...
sink(file.path(repDir,"Nt01.FastasSummary.txt"))
cat("\nFasta files summary\n\n")
print(rprt[o,])
...
pdf(file.path(repDir,"Nt01.FinalCovBoxplots.pdf"),paper="a4",width=5,height=10)
par(mfrow=c(2,1))
...
file.copy(file.path(repDir,"Nt01.FinalCovBoxplots.pdf"),
          file.path(resultsDir,"Nt01.FinalCovBoxplots.pdf"),
		  overwrite=TRUE)

file.copy(file.path(repDir,"Nt01.FastasSummary.txt"),
          file.path(resultsDir,"Nt01.FastasSummary.txt"),
		  overwrite=TRUE)
