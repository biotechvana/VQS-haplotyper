...
primers <- fread(file.path(dataDir,"primers.csv"), sep="auto", header=T,
                 stringsAsFactors=F)
###  Comprobar que todos los primers están descritos
...
for(i in 1:length(pools))
{
  ...
  seqs <- sread(sqq)
  seqs <- seqs[width(seqs)>=80]   # Elimina las que son demasiado cortas
  p.cv[i] <- length(seqs)
  
  ###  Loop sobre muestras en pool
  for(j in idx)
  {
    ###  primer up matches 
    #################################
