...
ylm <- c(0,max(res[,3])+max(res[,4]))
b <- barplot(res[,3],col="lavender",las=2,ylim=ylm,
             main="Average quality by fastq file")
arrows(x0=b,y0=res[,3]-res[,4],y1=res[,3]+res[,4],lwd=2,code=3,
       angle=90,len=0.1,col="blue")
